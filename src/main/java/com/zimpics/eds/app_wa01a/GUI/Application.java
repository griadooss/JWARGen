...
import com.zimpics.eds.app_wa01a.ULoad;
import com.zimpics.eds.app_wa01a.VDate;
...
public class Application extends JFrame {
    ...
    private JComponent createEditor() {
        ...
        JTextField anomalyHeader = new JTextField();
        anomalyHeader.setText("SPREADSHEET FILE DETAILS");
        Font font = new Font("Arial", Font.BOLD, 12);
        anomalyHeader.setFont(font);
        anomalyHeader.setBackground(Color.GRAY);
        anomalyHeader.setForeground(Color.WHITE);
        anomalyHeader.setToolTipText("Enter dates");
        anomalyHeader.setHorizontalAlignment(JTextField.CENTER);
        //anomalyHeader.setRows(2);
        panel.add(anomalyHeader, constraints);
    ...
    private void createNew() throws SQLException {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (FilenameUtils.getExtension(file.getName()).equals("xlsx")) {
                ...
                JOptionPane.showMessageDialog(this, "This application expects the file to be XLSX format! The file "
    ...
    private JToolBar createToolBar2() {
        ...
        toolBar2.add(validateAction);
        toolBar2.add(uploadAction);
        toolBar2.add(reportsAction);
        return toolBar2;
    }
    ...
    private void initComponents() {
        ...
        add(createToolBar2(), BorderLayout.PAGE_START);
    ...
    private void initMenu() {
        ...
        final JMenu editMenu = menuBar.add(new JMenu("Main Menu"));
    ...
    private boolean uploadFile() throws SQLException {
        if (selectedFile == null) {
            JOptionPane.showMessageDialog(this,
                    "Please select a file to upload.",
                    "Selection required!", JOptionPane.INFORMATION_MESSAGE);
        } else if (DbHelper.checkFileStatus(selectedFile.getfName()).equals("VALID")) {
            try {
                return ULoad.upload(selectedFile);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        } else {
            JOptionPane.showMessageDialog(this,
                    "File must have a status of 'VALID' before it can be uploaded to the database!",
                    "Validation Needed", JOptionPane.INFORMATION_MESSAGE);
        }
        refreshFileData();
        return false;
    }
    ...
    private void validateFile() throws SQLException {
        if (selectedFile == null) {
            JOptionPane.showMessageDialog(this,
                    "Please select a file to validate.",
                    "Selection required!", JOptionPane.INFORMATION_MESSAGE);
        } else if (!DbHelper.checkFileStatus(selectedFile.getfName()).equals("VALID")) {
            VDate.validate(selectedFile);
        } else if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(this,
                "File has already been validated! Are you sure you want to validate it again??",
                "RE-Validate File??", JOptionPane.YES_NO_OPTION)) {
            VDate.validate(selectedFile);
        }
    }
   ...
   /*     try {
    ...
    private void runReports() {
        SwingUtilities.invokeLater(() -> {
            ...
            // Application.LOGGER.debug("Starting report form");

            reportForm.setTitle("Reporting and Client Maintenance");
            reportForm.setSize(650, 425);
